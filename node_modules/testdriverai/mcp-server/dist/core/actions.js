...
export class NoActiveSessionError extends Error {
    code;
    expiredSessionId;
    constructor(code, message, expiredSessionId) {
        super(message);
 ...
 */
async function loadTestDriverSdk() {
    ...
    try {
        ...
        return (await import(/* @vite-ignore */ rel));
    }
}
/**
...
export async function find(description, timeout) {
    ...
    if (raw.croppedImage) {
        ...
        if (shot)
            images.push({ kind: "screenshot", base64: shot });
    }
    const data = leanResponse(raw);
    return {
        ok: found,
        text: found
            ? `Found: "${description}" at (${raw.coordinates?.x}, ${raw.coordinates?.y})\nRef: ${ref}`
...
export function click(ref, action = "click") {
    return actOnRef(ref, action, "click");
}
export function hover(ref) {
...
export async function findAndClick(description, action = "click") {
    ...
    if (!found) {
        ...
        const images = [];
        if (raw.croppedImage)
            images.push({ kind: "screenshot", base64: bareBase64(raw.croppedImage) });
        else {
            const shot = await captureScreen();
            if (shot)
                images.push({ kind: "screenshot", base64: shot });
    ...
    return {
        ok: true,
        text: `Found and clicked: "${description}" at (${raw.coordinates?.x}, ${raw.coordinates?.y})\nRef: ${ref}`,
        data: {
            ...data,
...
export async function check(task, referenceImage) {
    ...
    const hasCodeBlocks = aiResponse && (aiResponse.includes("```yml") || aiResponse.includes("```yaml") || aiResponse.includes("- command:"));
